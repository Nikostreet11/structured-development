...
int main(void)
{
	...
	GradeNode *st_dist = compute_distribution(mapped, n);
	printf("\nGrade Distribution:\n");
	print_distribution(st_dist);
	free_distribution(st_dist);

	map_scores(scores, n, pm_mapper, mapped);
	printf("\nMapped grades:\n");
	print_grades(scores, mapped, n);

	GradeNode *pm_dist = compute_distribution(mapped, n);
	printf("\nGrade Distribution:\n");
	print_distribution(pm_dist);
	free_distribution(pm_dist);

	map_scores(scores, n, pf_mapper, mapped);
	printf("\nMapped grades:\n");
	print_grades(scores, mapped, n);

	GradeNode *pf_dist = compute_distribution(mapped, n);
	printf("\nGrade Distribution:\n");
	print_distribution(pf_dist);
	free_distribution(pf_dist);
